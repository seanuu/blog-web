    ...
    <v-app dark>
        <div class="register">
            ...
            <v-form ref="form" class="register-form" v-model="valid" lazy-validation>
                <v-text-field label="username" :rules="usernameRules" ref="password" required placeholder="用户名"
                              @input="checkUsername" validate-on-blur @keydown.enter="register"
                              v-model="user.username"></v-text-field>

                <v-text-field label="password" :rules="passwordRules" required placeholder="密码"
                              type="password" validate-on-blur @keydown.enter="register"
                              v-model="user.password"></v-text-field>

                <v-text-field label="verification code" :rules="verificationCodeRules" required
                              placeholder="验证码" validate-on-blur @keydown.enter="register"
                              v-model="user.verificationCode"></v-text-field>

                <v-btn class="register-form-button" block color="secondary" style="font-size: 12px"
        ...
        data: function () {
            return {
                ...
                usernameRules: [
                    ...
                    v => !this.usernameStatus || 'Username already exist'
                ],
                passwordRules: [
